...
def main(args, config, build_model):
    ...
    # setattr(runner.model,"teacher_model",runner.teacher_model)
    # params_container = dict()
    # for name, param in runner.model.named_parameters():
        
    #     params_container[name]=param.clone().detach()
    # from pdb import set_trace
    # set_trace()
    # torch.save(params_container,'./cvpods_model.pth')
    ...
    if cfg.TRAINER.WITH_TEACHER:
        extra_hooks.append(hooks.MeanTeacher(runner,
                                                momentum=0.999,
                                                interval=1,
                                                warm_up=0,
    ...
    if args.gpus_temp_monitor:
        gpus_id = os.environ.get("CUDA_VISIBLE_DEVICES")
        gpus_id_int = [int(i) for i in gpus_id.split(',')]
        extra_hooks.append(hooks.gpus_temp_monitor(gpus=gpus_id_int,
                                                    max_temp=75,
                                                    stop_time=30))
