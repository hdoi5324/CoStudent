...
def test_argument_parser():
    ...
    parser.add_argument("--TEACHER_WEIGHTS", type=str, default=None, help="")
...
def main(args, config, build_model):
    ...
    # set_trace()
    config.LOGGER_NAME = "test_log.txt"
    ...
    for current_file in valid_files:
        ...
        if cfg.MODEL.FCOS.WITH_TEACHER:
            model,teacher_model = build_model(cfg)
        else:
        ...
        if teacher_model is not None:
            weight = args.TEACHER_WEIGHTS
            DefaultCheckpointer(teacher_model, save_dir=cfg.OUTPUT_DIR).resume_or_load(
                weight, resume=args.resume
            )
