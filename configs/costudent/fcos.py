...
def Revision_PRED(Instance1,Instance2):
    ...
    while(True):
        ...
        refine_gt_inds = (ious > 0.5).any(dim=0) # Columns with a match > 0.5 (ie by Instance2)
        refine_inds = ious.max(dim=0)[1] # Index of row by column with highest overlap

        refine_pred_scores = scores1[refine_inds] # scores of box with largest overlap
        need_refine = refine_pred_scores >= scores2 # is it greater than the instance2 ie Instance2 needs refining

        lower_scores_inds = (~refine_gt_inds | ~need_refine) & refine_gt_inds # iou > 0.5 but score is lower in instance 1. could be ~needs_refine & refine_gt_inds

        lower_scores_inds0 = torch.where(lower_scores_inds)[0] # column numbers. 

        if lower_scores_inds0.numel()>0:
            index = [refine_inds[lower_scores_inds],lower_scores_inds0] # col/row locations where iou > 0.5 but score is lower in instance1
    ...
    refine_inds = refine_inds[refine_gt_inds] # For the columsn with a refinement candidate (ie instance1 > 0.5 and score1>score2) find the index of instance1 that will be largest iou.
    refine_gt_inds = torch.where(refine_gt_inds)[0] # get index where True ie instance2 that needs refinement and has candidate
...
class FCOS(nn.Module):
    ...
    def fcos_losses1(self, gt_classes, gt_shifts_deltas, gt_centerness,
        ...
        loss_box_reg = iou_loss(
            ...
            box_mode="ltrb",
            loss_type=self.iou_loss_type if self.iou_loss_type != "dn_iou" else "giou",
            reduction="mean",
        )
        if self.iou_loss_type == "dn_iou":
            loss_box_reg += dn_diou_loss(
                pred_shift_deltas[foreground_idxs],
                gt_shifts_deltas[foreground_idxs],
                gt_centerness[foreground_idxs],
                reduction="mean",
