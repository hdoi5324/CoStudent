...
class ColorJiterTransform(Transform):
    def __init__(self, scale_factor, backend):
        super().__init__()
        self._set_attributes(locals())
    ...
    def apply_image(self, img, interp=None):
        ...
        color_jitter_img = transforms.RandomApply([color_jitter], p=0.8)(Image.fromarray(img))
        img_aug = transforms.ToTensor()(color_jitter_img).permute(1,2,0) * 255
        img_aug = np.array(img_aug).astype(np.uint8)
        return img_aug
